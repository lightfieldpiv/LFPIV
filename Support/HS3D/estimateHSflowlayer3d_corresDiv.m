function uv = estimateHSflowlayer3d_corresDiv(source,target,uv,lambda,uv_corres,F)
% maxwarping
div_para =0;
...
% reshape uv_c
u_vector = reshape(uv_corres(:,:,:,1),H*w*D,1);
v_vector = reshape(uv_corres(:,:,:,2),H*w*D,1);
w_vector = reshape(uv_corres(:,:,:,3),H*w*D,1);
...
%           Iz*It+lambda*L*W;];
    ...
    A = [Ix*Ix+lambda*L+lambda_corres*F+lambda_div*F_div Ix*Iy Ix*Iz;...
        Ix*Iy Iy*Iy+lambda*L+lambda_corres*F+lambda_div*F_div Iy*Iz;...
        Ix*Iz Iy*Iz Iz*Iz+lambda*L+lambda_corres*F+lambda_div*F_div];
    
    b = -[Ix*It+lambda*L*U+lambda_corres*F*U-lambda_corres*F*u_vector + lambda_div*F_div*U-lambda_div*F_div*u0;...
          Iy*It+lambda*L*V+lambda_corres*F*V-lambda_corres*F*v_vector + lambda_div*F_div*V-lambda_div*F_div*v0;...
          Iz*It+lambda*L*W+lambda_corres*F*W-lambda_corres*F*w_vector + lambda_div*F_div*W-lambda_div*F_div*w0;];
